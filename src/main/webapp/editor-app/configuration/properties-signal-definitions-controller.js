...
angular.module('activitiModeler').controller('ActivitiSignalDefinitionsCtrl',
		['$scope', '$modal', function($scope, $modal) {

			// Config for the modal window
			var opts = {
				template : 'editor-app/configuration/properties/signal-definitions-popup.html?version='
						+ Date.now(),
				scope : $scope
			};
				...
				function($scope, $q, $translate, $timeout) {
					...
					if ($scope.property.value !== undefined
						...
						if ($scope.property.value.constructor == String) {
							$scope.signalDefinitions = JSON
									.parse($scope.property.value);
						} else {
							// Note that we clone the json object rather then
							// setting it directly,
							// this to cope with the fact that the user can
							// click the cancel button and no changes should
							// have happened
							$scope.signalDefinitions = angular
									.copy($scope.property.value);
						}
					...
					// Array to contain selected signal definitions (yes - we
					// only can select one, but ng-grid isn't smart enough)
					...
					$q.all([idPromise, namePromise, scopePromise]).then(
							function(results) {
					...
					$scope.addNewSignalDefinition = function() {
						...
						$timeout(function() {
									$scope.gridOptions
											.selectItem(
													$scope.signalDefinitions.length
															- 1, true);
								});
					};
					...
					$scope.removeSignalDefinition = function() {
						if ($scope.selectedSignals
								&& $scope.selectedSignals.length > 0) {
							var index = $scope.signalDefinitions
									.indexOf($scope.selectedSignals[0]);
