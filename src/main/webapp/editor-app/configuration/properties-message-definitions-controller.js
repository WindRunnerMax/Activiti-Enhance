...
angular.module('activitiModeler').controller('ActivitiMessageDefinitionsCtrl',
		['$scope', '$modal', function($scope, $modal) {

			// Config for the modal window
			var opts = {
				template : 'editor-app/configuration/properties/message-definitions-popup.html?version='
						+ Date.now(),
				scope : $scope
			};
				...
				function($scope, $q, $translate, $timeout) {
					...
					if ($scope.property.value !== undefined
						...
						if ($scope.property.value.constructor == String) {
							...
							// have happened
							$scope.messageDefinitions = angular
					...
					// Array to contain selected mesage definitions (yes - we
					...
					$scope.addNewMessageDefinition = function() {
						...
						$timeout(function() {
							$scope.gridOptions.selectItem(
									$scope.messageDefinitions.length - 1, true);
						});
					};

					// Click handler for remove button
					$scope.removeMessageDefinition = function() {
						if ($scope.selectedMessages
								&& $scope.selectedMessages.length > 0) {
							var index = $scope.messageDefinitions
									.indexOf($scope.selectedMessages[0]);
