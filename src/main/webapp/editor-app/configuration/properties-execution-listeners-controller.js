...
var KisBpmExecutionListenersCtrl = ['$scope', '$modal', '$timeout',
		'$translate', function($scope, $modal, $timeout, $translate) {

			// Config for the modal window
			var opts = {
				template : 'editor-app/configuration/properties/execution-listeners-popup.html?version='
						+ Date.now(),
				scope : $scope
			};
...
var KisBpmExecutionListenersPopupCtrl = ['$scope', '$q', '$translate',
		function($scope, $q, $translate) {
			...
			if ($scope.property.value !== undefined
				...
				if ($scope.property.value.executionListeners.constructor == String) {
					$scope.executionListeners = JSON
							.parse($scope.property.value.executionListeners);
				} else {
					// Note that we clone the json object rather then setting it
					// directly,
					// this to cope with the fact that the user can click the
					// cancel button and no changes should have happened
					$scope.executionListeners = angular
							.copy($scope.property.value.executionListeners);
				}

				for (var i = 0; i < $scope.executionListeners.length; i++) {
					var executionListener = $scope.executionListeners[i];
					if (executionListener.className !== undefined
							&& executionListener.className !== '') {
						executionListener.implementation = executionListener.className;
					} else if (executionListener.expression !== undefined
							&& executionListener.expression !== '') {
						executionListener.implementation = executionListener.expression;
					} else if (executionListener.delegateExpression !== undefined
							&& executionListener.delegateExpression !== '') {
			...
			// Array to contain selected properties (yes - we only can select
			// one, but ng-grid isn't smart enough)
			...
			$q.all([eventPromise, implementationPromise, namePromise]).then(
					function(results) {
			...
			$scope.removeListener = function() {
				if ($scope.selectedListeners.length > 0) {
					var index = $scope.executionListeners
							.indexOf($scope.selectedListeners[0]);
			...
			$scope.moveListenerUp = function() {
				if ($scope.selectedListeners.length > 0) {
					var index = $scope.executionListeners
							.indexOf($scope.selectedListeners[0]);
					if (index != 0) { // If it's the first, no moving up of
										// course
						// Reason for funny way of swapping, see
						// https://github.com/angular-ui/ng-grid/issues/272
						var temp = $scope.executionListeners[index];
						$scope.executionListeners.splice(index, 1);
						$timeout(function() {
									$scope.executionListeners.splice(
											index + -1, 0, temp);
								}, 100);
			...
			$scope.moveListenerDown = function() {
				if ($scope.selectedListeners.length > 0) {
					var index = $scope.executionListeners
							.indexOf($scope.selectedListeners[0]);
					if (index != $scope.executionListeners.length - 1) { // If
																			// it's
																			// the
																			// last
																			// element,
			...
			$scope.removeField = function() {
				if ($scope.selectedFields.length > 0) {
					var index = $scope.selectedListeners[0].fields
							.indexOf($scope.selectedFields[0]);
			...
			$scope.moveFieldUp = function() {
				if ($scope.selectedFields.length > 0) {
					...
					if (index != 0) { // If it's the first, no moving up of
						...
						$scope.selectedListeners[0].fields.splice(index, 1);
						$timeout(function() {
									$scope.selectedListeners[0].fields.splice(
											index + -1, 0, temp);
								}, 100);
			...
			$scope.moveFieldDown = function() {
				if ($scope.selectedFields.length > 0) {
					var index = $scope.selectedListeners[0].fields
							.indexOf($scope.selectedFields[0]);
					if (index != $scope.selectedListeners[0].fields.length - 1) { // If
																					// it's
																					// the
																					// last
																					// element,
