...
var KisBpmTaskListenersCtrl = ['$scope', '$modal', '$timeout', '$translate',
		function($scope, $modal, $timeout, $translate) {

			// Config for the modal window
			var opts = {
				template : 'editor-app/configuration/properties/task-listeners-popup.html?version='
						+ Date.now(),
				scope : $scope
			};
...
var KisBpmTaskListenersPopupCtrl = ['$scope', '$q', '$translate',
		function($scope, $q, $translate) {
			...
			if ($scope.property.value !== undefined
				...
				if ($scope.property.value.taskListeners.constructor == String) {
					$scope.taskListeners = JSON
							.parse($scope.property.value.taskListeners);
				} else {
					...
					$scope.taskListeners = angular
							.copy($scope.property.value.taskListeners);
				}

				for (var i = 0; i < $scope.taskListeners.length; i++) {
					var taskListener = $scope.taskListeners[i];
					if (taskListener.className !== undefined
							&& taskListener.className !== '') {
						taskListener.implementation = taskListener.className;
					} else if (taskListener.expression !== undefined
							&& taskListener.expression !== '') {
						taskListener.implementation = taskListener.expression;
					} else if (taskListener.delegateExpression !== undefined
							&& taskListener.delegateExpression !== '') {
			...
			$scope.moveListenerUp = function() {
				if ($scope.selectedListeners.length > 0) {
					...
					if (index != 0) { // If it's the first, no moving up of
						...
						$scope.taskListeners.splice(index, 1);
						$timeout(function() {
									$scope.taskListeners.splice(index + -1, 0,
											temp);
								}, 100);
			...
			$scope.moveListenerDown = function() {
				if ($scope.selectedListeners.length > 0) {
					var index = $scope.taskListeners
							.indexOf($scope.selectedListeners[0]);
					if (index != $scope.taskListeners.length - 1) { // If it's
																	// the last
																	// element,
																	// no moving
																	// down of
