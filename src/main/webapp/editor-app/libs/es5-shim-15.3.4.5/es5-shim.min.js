(function(definition) {
		...
		if (function() {
			...
			Array.prototype.splice = function(start, deleteCount) {
				if (!arguments.length) {
					...
					return array_splice.apply(this, [
									...
									deleteCount === void 0 ? this.length
											- start : deleteCount]
			...
			Array.prototype.splice = function(start, deleteCount) {
				...
				if (addElementsCount > 0) {
					...
					args.unshift
							.apply(args, _Array_slice_.call(this, 0, start));
	...
	if (!Date.prototype.toISOString
		...
		Date.prototype.toISOString = function toISOString() {
			...
			year = (year < 0 ? "-" : year > 9999 ? "+" : "")
					+ ("00000" + Math.abs(year)).slice(0 <= year
							&& year <= 9999 ? -4 : -6);
			...
			Date.parse = function parse(string) {
				...
				if (match) {
					var year = Number(match[1]), month = Number(match[2] || 1)
							...
							|| 0)
							* 1e3), offset = !match[4] || match[8]
							? 0
							: Number(new NativeDate(1970, 0)), signOffset = match[9] === "-"
							? 1
							: -1, hourOffset = Number(match[10] || 0), minuteOffset = Number(match[11]
							|| 0), result;
	...
	if (!Number.prototype.toFixed || 8e-5.toFixed(3) !== "0.000"
			|| .9.toFixed(0) === "0" || 1.255.toFixed(2) !== "1.25"
...
(function() {
			...
			String.prototype.split = function(separator, limit) {
				...
				var output = [], flags = (separator.ignoreCase ? "i" : "")
						...
						+ (separator.sticky ? "y" : ""), lastLastIndex = 0, separator = new RegExp(
						separator.source, flags + "g"), separator2, match, lastIndex, lastLength;
				string += "";
