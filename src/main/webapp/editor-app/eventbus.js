...
KISBPM.eventBus = {
	...
	/**
	 * Add an event listener to the event bus, listening to the event with the
	 * provided type. Type and callback are mandatory parameters.
	 * 
		...
		if (typeof this.listeners[type] != "undefined") {
			...
			for (var i = 0; i < numOfCallbacks; i++) {
				...
				if (listener.scope === scope && listener.callback === callback) {
					// Do nothing, this is the listener and doesn't need to
					// survive
				} else {
	...
	/**
	 * Dispatch an event to all event listeners registered to that specific
	 * type.
	 */
