...
(function($) {
	...
	fancybox_get_zoom_to = function() {
		...
		horizontal_space = (shadow + margin) * 2, vertical_space = (shadow + margin)
				* 2, double_padding = (currentOpts.padding * 2),
		...
		if (resize
			...
			if (selectedOpts.type == 'image' || selectedOpts.type == 'swf') {
				...
				ratio = Math.min(Math.min(view[0] - horizontal_space,
								currentOpts.width)
								/ currentOpts.width, Math.min(view[1]
										- vertical_space, currentOpts.height)
								/ currentOpts.height);
	...
	fancybox_process_title = function() {
		...
		title = $.isFunction(currentOpts.titleFormat)
				? currentOpts.titleFormat(title, currentArray, currentIndex,
						currentOpts)
	...
	fancybox_draw = function(pos) {
		...
		height = Math.max(height - (currentOpts.padding * 2 + (titleh * pos)),
				0);
	...
	fancybox_show = function() {
		...
		if (currentOpts.transitionIn == 'elastic') {
			...
			inner.css({
				...
				left : currentOpts.padding,
				width : Math
						.max(start_pos.width - (currentOpts.padding * 2), 1),
				height : Math.max(start_pos.height - (currentOpts.padding * 2),
						1)
			}).html(tmp.contents());
			...
			inner.css({
				...
				left : currentOpts.padding,
				width : Math
						.max(final_pos.width - (currentOpts.padding * 2), 1),
				height : Math.max(final_pos.height - (currentOpts.padding * 2)
								- titleh, 1)
			}).html(tmp.contents());

			wrap.css(final_pos).fadeIn(
					currentOpts.transitionIn == 'none'
							? 0
