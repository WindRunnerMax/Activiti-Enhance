	...
	d.widget("ech.multiselect", {
		...
		_create : function() {
			...
			a = (this.button = d('<button type="button"><span class="ui-icon ui-icon-triangle-2-n-s"></span></button>'))
					...
					.addClass(b.classes).attr({
								title : a.attr("title"),
								"aria-haspopup" : true
							}).insertAfter(a);
			(this.buttonlabel = d("<span />")).html(b.noneSelectedText)
					.appendTo(a);
			a = (this.menu = d("<div />"))
					.addClass("ui-multiselect-menu ui-widget ui-widget-content ui-corner-all")
					.addClass(b.classes).insertAfter(a);
			var e = (this.header = d("<div />"))
					...
					.html(function() {
						return b.header === true
								? '<li><a class="ui-multiselect-all" href="#"><span class="ui-icon ui-icon-check"></span><span>'
										...
										+ b.uncheckAllText + "</span></a></li>"
								: typeof b.header === "string" ? "<li>"
										+ b.header + "</li>" : ""
					})
					.append('<li class="ui-multiselect-close"><a href="#" class="ui-multiselect-close"><span class="ui-icon ui-icon-circle-close"></span></a></li>')
					.appendTo(e);
			(this.checkboxContainer = d("<ul />"))
		...
		refresh : function(a) {
			var b = this.options, e = this.menu, c = this.button, g = this.checkboxContainer, f = [], h = this.element
					.attr("id")
					|| o++;
			g.empty();
			this.element.find("option").each(function(k) {
				...
				if (n.length > 0) {
					...
					j = d("<li />")
							.addClass(i ? "ui-multiselect-disabled" : "")
							.appendTo(g);
					l = d("<label />").attr("for", k).addClass(l.join(" "))
							.appendTo(j);
					d('<input type="' + (b.multiple ? "checkbox" : "radio")
							+ '" ' + (this.selected ? 'checked="checked"' : "")
							+ ' name="multiselect_' + h + '" />').attr({
								id : k,
		...
		update : function() {
			var a = this.options, b = this.labels.find("input"), e = b
					.filter(":checked"), c = e.length;
			a = c === 0 ? a.noneSelectedText : d.isFunction(a.selectedText)
					? a.selectedText.call(this, c, b.length, e.get())
					: /\d/.test(a.selectedList) && a.selectedList > 0
							&& c <= a.selectedList ? e.map(function() {
								return this.title
							}).get().join(", ") : a.selectedText
							.replace("#", c).replace("#", b.length);
		...
		_bindEvents : function() {
			...
			this.header.delegate("a", "click.multiselect", function(c) {
						d(this).hasClass("ui-multiselect-close")
								? b.close()
								: b[d(this).hasClass("ui-multiselect-all")
										? "checkAll"
										: "uncheckAll"]();
						c.preventDefault()
					});
			this.menu.delegate("li.ui-multiselect-optgroup-label a",
					"click.multiselect", function(c) {
						c.preventDefault();
						var g = d(this), f = g.parent()
						...
						if (!d(this).hasClass("ui-state-disabled")) {
							...
							d(this).addClass("ui-state-hover").find("input")
									.focus()
						}
					...
					"click.multiselect", function(c) {
						var g = d(this), f = this.value, h = this.checked, k = b.element
								.find("option");
			...
			d(document).bind("mousedown.multiselect", function(c) {
				b._isOpen && !d.contains(b.menu[0], c.target)
						&& c.target !== b.button[0] && b.close()
			});
		...
		_setMenuWidth : function() {
			var a = this.menu, b = this.button.outerWidth()
					- parseInt(a.css("padding-left"), 10)
					- parseInt(a.css("padding-right"), 10)
					- parseInt(a.css("border-right-width"), 10)
					- parseInt(a.css("border-left-width"), 10);
			a.width(b || this.button.outerWidth())
		},
		_traverse : function(a, b) {
			var e = d(b), c = a === 38 || a === 37;
			e = e.parent()[c ? "prevAll" : "nextAll"]("li:not(.ui-multiselect-disabled, .ui-multiselect-optgroup-label)")[c
					? "last"
					: "first"]();
			if (e.length)
				...
				this.menu.find("label")[c ? "last" : "first"]()
						.trigger("mouseover");
		...
		open : function() {
			...
			if (!(this._trigger("beforeopen") === false
					|| a.hasClass("ui-state-disabled") || this._isOpen)) {
		...
		_setOption : function(a, b) {
			...
			switch (a) {
				...
				case "noneSelectedText" :
					...
					break;
				case "classes" :
					e.add(this.button).removeClass(this.options.classes)
							.addClass(b)
			}
