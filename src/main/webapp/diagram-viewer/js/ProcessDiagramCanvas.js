...
ProcessDiagramCanvas.prototype = {
	...
	drawCatchingSignalEvent : function(x, y, width, height, isInterrupting,
			name) {
		this.g.setStart();
		this._drawCatchingEvent(x, y, width, height, isInterrupting, null);
	...
	drawCatchingMessageEvent : function(x, y, width, height, isInterrupting,
			name) {
		this.g.setStart();
		this._drawCatchingEvent(x, y, width, height, isInterrupting, null);
	...
	drawCatchingMultipleEvent : function(x, y, width, height, isInterrupting,
			name) {
		this.g.setStart();
		this._drawCatchingEvent(x, y, width, height, isInterrupting, null);
	...
	_drawFlow : function(waypoints, conditional, isDefault, highLighted,
			withArrowHead, connectionType) {
		var originalPaint = this.getPaint();
		...
		if (flowName) {
			...
			var textX = xPointArray[0] < xPointArray[1]
					? xPointArray[0]
					: xPointArray[1];
			var textY = yPointArray[0] < yPointArray[1]
					? yPointArray[1]
					: yPointArray[0];
			// fix xy
			textX += 20;
		...
		polyClone.click(function() {
			...
			if (contextObject) {
				console.log("[flow], objectId: " + object.id + ", flow: "
						+ contextObject.flow);
	...
	drawSequenceflow : function(waypoints, conditional, isDefault, highLighted) {
		...
		this._drawFlow(waypoints, conditional, isDefault, highLighted,
				withArrowHead, CONNECTION_TYPE.SEQUENCE_FLOW);
	},
	...
	drawMessageflow : function(waypoints, highLighted) {
		...
		this._drawFlow(waypoints, conditional, isDefault, highLighted,
				withArrowHead, CONNECTION_TYPE.MESSAGE_FLOW);
	},
	...
	drawAssociation : function(waypoints, withArrowHead, highLighted) {
		...
		var conditional = isDefault = false;
		this._drawFlow(waypoints, conditional, isDefault, highLighted,
				withArrowHead, CONNECTION_TYPE.ASSOCIATION);
	},
	 ...
	 * "z"); //var c = DefaultProcessDiagramCanvas.g.ellipse(0, 0, 3, 3);
	 ...
	 * this.getPaint());
	 *  / * // shaddow var c0 = arrowHead.clone(); c0.transform("...t-1 1");
	 * c0.attr("stroke-width", this.strokeWidth); c0.attr("stroke",
	 * Color.black); c0.attr("opacity", 0.15); c0.toBack(); / },
	 */
	...
	drawSequenceflowWithoutArrow : function(waypoints, conditional, isDefault,
			highLighted) {
	...
	_drawTask : function(name, x, y, width, height, thickBorder) {
		...
		if (name) {
			...
			var boxY = y + height / 2 - boxHeight / 2 + ICON_PADDING
					+ ICON_PADDING - 2 - 2;
			/*
	...
	drawUserTask : function(name, x, y, width, height) {
		...
		var img = this.g.image(USERTASK_IMAGE, x + ICON_PADDING, y
	...
	drawExpandedSubProcess : function(name, x, y, width, height,
			isTriggeredByEvent) {
		this.g.setStart();
		// anti smoothing
		if (this.strokeWidth % 2 == 1)
		...
		var textElement = this.g.text(
				x + width / 2 - realWidth * 0 / 2 + 0 * paddingX,
				y + realHeight / 2 + paddingY, truncated).attr(fontAttr);
	...
	drawCollapsedSubProcess : function(name, x, y, width, height,
			isTriggeredByEvent) {
		this.g.setStart();
	...
	drawCollapsedMarker : function(x, y, width, height) {
		...
		var rect = this.g.rect(x + (width - rectangleWidth) / 2, y + height
		...
		var line = this.g.path("M" + cx + " " + (cy + 2) + "L" + cx + " "
						+ (cy - 2) + "M" + (cx - 2) + " " + cy + "L" + (cx + 2)
						+ " " + cy).attr({
					"stroke-width" : this.strokeWidth
				});
	...
	drawActivityMarkers : function(x, y, width, height,
			multiInstanceSequential, multiInstanceParallel, collapsed) {
		if (collapsed) {
			if (!multiInstanceSequential && !multiInstanceParallel) {
				this.drawCollapsedMarker(x, y, width, height);
			} else {
				this.drawCollapsedMarker(x - MARKER_WIDTH / 2 - 2, y, width,
						height);
				if (multiInstanceSequential) {
					console.log("is collapsed and multiInstanceSequential");
					this.drawMultiInstanceMarker(true,
							x + MARKER_WIDTH / 2 + 2, y, width, height);
				} else if (multiInstanceParallel) {
					console.log("is collapsed and multiInstanceParallel");
					this.drawMultiInstanceMarker(false, x + MARKER_WIDTH / 2
									+ 2, y, width, height);
				}
			}
		} else {
	...
	drawMultiInstanceMarker : function(sequential, x, y, width, height) {
		...
		if (sequential) {
			var line = this.g.path("M" + lineX + " " + lineY + "L"
							+ (lineX + rectangleWidth) + " " + lineY + "M"
							+ lineX + " " + (lineY + rectangleHeight / 2) + "L"
							+ (lineX + rectangleWidth) + " "
							+ (lineY + rectangleHeight / 2) + "M" + lineX + " "
							+ (lineY + rectangleHeight) + "L"
							+ (lineX + rectangleWidth) + " "
							+ (lineY + rectangleHeight)).attr({
						"stroke-width" : this.strokeWidth
					});
		} else {
			var line = this.g.path("M" + lineX + " " + lineY + "L" + lineX
							+ " " + (lineY + rectangleHeight) + "M"
							+ (lineX + rectangleWidth / 2) + " " + lineY + "L"
							+ (lineX + rectangleWidth / 2) + " "
							+ (lineY + rectangleHeight) + "M"
							+ (lineX + rectangleWidth) + " " + lineY + "L"
							+ (lineX + rectangleWidth) + " "
							+ (lineY + rectangleHeight)).attr({
						"stroke-width" : this.strokeWidth
					});
	...
	drawHighLight : function(x, y, width, height) {
		...
		var rect = this.g.rect(x - THICK_TASK_BORDER_STROKE, y
						- THICK_TASK_BORDER_STROKE, width
						+ THICK_TASK_BORDER_STROKE * 2, height
						+ THICK_TASK_BORDER_STROKE * 2, TASK_CORNER_ROUND);
	...
	drawLabel111111111 : function(text, x, y, width, height, labelAttrs) {
		...
		if (text != null && text != undefined && text != "") {
			...
			var label = this.g.text(textX, textY, wrappedText).attr(attr)
					.attr(textAttrs);
