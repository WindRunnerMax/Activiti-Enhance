window.onload = function() {
	...
	// var curve = paper.ellipse(100, 100, 1, 1).attr({"stroke-width": 0, fill:
	// Color.red});
	...
	var AttributedStringIterator = function(text) {
		...
		while (i < 1000) {
			...
			// //console.log("fullPos: " + fullPos + ", pos: " + pos + ",
			// string: ", string);
		...
		preceding : function(pos) {
			...
			for (var i in this.startWordOffsets) {
				...
				if (pos < startWordOffset && i > 0) {
					// console.log("startWordOffset: " +
					// this.startWordOffsets[i-1]);
					// console.groupEnd();
	...
	/*
	 * var TextMeasurer = function(paper, text, fontAttrs){ this.text = text;
	 * this.paper = paper; this.fontAttrs = fontAttrs;
	 * 
	 * this.fStart = this.text.getBeginIndex();
	 *  }; TextMeasurer.prototype = { getLineBreakIndex: function(start,
	 * maxAdvance){ var localStart = start - this.fStart; }, getLayout:
	 * function(){ } }
	 */
	...
	var LineBreakMeasurer = function(paper, text, fontAttrs) {
		...
		this.rafaelTextObject = this.paper.text(100, 100, this.text.text)
				.attr(fontAttrs).attr("text-anchor", "start");
	...
	LineBreakMeasurer.prototype = {
		nextOffset : function(wrappingWidth, offsetLimit, requireNextWord) {
			...
			if (this.pos < this.limit) {
				...
				var charAtMaxAdvance = this.getLineBreakIndex(this.pos,
						wrappingWidth);
				// charAtMaxAdvance --;
				// console.log("charAtMaxAdvance:", charAtMaxAdvance, ", [" +
				// this.text.getCharAtPos(charAtMaxAdvance) + "]");
				...
				if (charAtMaxAdvance == this.limit) {
					...
					/*
					 * var testPos = charAtMaxAdvance + 1; if (testPos ==
					 * this.limit) { console.error("hbz..."); } else {
		...
		nextLayout : function(wrappingWidth) {
			...
			if (this.pos < this.limit) {
				...
				var layoutLimit = this.nextOffset(wrappingWidth, this.limit,
						requireNextWord);
		...
		getLineBreakIndex : function(pos, wrappingWidth) {
			...
			try {
				...
				// var dot = this.paper.ellipse(svgPoint.x, svgPoint.y, 1,
				// 1).attr({"stroke-width": 0, fill: Color.blue});
				svgPoint.x = svgPoint.x + wrappingWidth;
				// svgPoint.y = bb.y;
				// console.log("svgPoint:", svgPoint);

				// var dot = this.paper.ellipse(svgPoint.x, svgPoint.y, 1,
				// 1).attr({"stroke-width": 0, fill: Color.red});

				charNum = this.svgTextObject.getCharNumAtPosition(svgPoint);
			} catch (e) {
				console.warn("getStartPositionOfChar error, pos:" + pos);
				/*
				 * var testPos = pos + 1; if (testPos < this.limit) { return
				 * testPos }
				 */
			}
			...
			if (charNum == -1) {
				...
				// When case there is new line between pos and charnum then use
				// this new line
				var newLineIndex = this.text.getEndIndex(pos);
				if (newLineIndex < charNum) {
					console.log("newLineIndex <= charNum, newLineIndex:"
									+ newLineIndex + ", charNum:" + charNum,
							"\""
									+ this.text.text.substr(newLineIndex + 1)
											.replace(/\n/g, "↵") + "\"");
					// console.groupEnd();
				...
				// var charAtMaxAdvance = this.text.text.substring(charNum,
				// charNum + 1);
	...
	function drawMultilineText(text, x, y, boxWidth, boxHeight, options) {
		...
		if (measurer.rafaelTextObject.attr("text-anchor") == "middle")
			measurer.rafaelTextObject.attr("x", x + boxWidth / 2 + TEXT_PADDING
							/ 2);
		else if (measurer.rafaelTextObject.attr("text-anchor") == "end")
			measurer.rafaelTextObject
					.attr("x", x + boxWidth + TEXT_PADDING / 2);
		else
			measurer.rafaelTextObject.attr("x", x + boxWidth / 2 - bb.width / 2
							+ TEXT_PADDING / 2);
		...
		/*
		 * var box = paper.rect(x+.0 + boxWidth/2 - bb.width/2+ TEXT_PADDING/2,
		 * y + .5 + boxHeight/2 - bb.height/2, width, height).attr(boxStyle);
		 * box.attr("height", bb.height);
		 */
		// var box = paper.rect(bb.x - .5 + bb.width/2 + TEXT_PADDING, bb.y +
		// bb.height/2, bb.width, bb.height).attr(boxStyle);
		...
		var dotCenter = paper.ellipse(textAreaCX, textAreaCY, 3, 3).attr({
		...
		var rect = paper.rect(x + .5, y + .5, boxWidth, boxHeight)
				.attr(boxStyle);
	}

	/*
	 * for (var i=0; i<1; i++) { var t = text; //var t = "Высококвалифицирова";
	 * 
	 * var text = paper.text(300, 100, t).attr(font).attr("text-anchor",
	 * "start"); var bbText = text.getBBox(); paper.rect(300+.5, 100 + .5,
	 * bbText.width, bbText.height).attr({"stroke-width": 1}); console.log("t: ",
	 * t.replace(/\n/g, "↵"));
	 * 
	 * while (measurer.getPosition() < measurer.text.getEndIndex()) { var layout =
	 * measurer.nextLayout(width); //console.log("LAYOUT: " + layout + ",
	 * getPosition: " + measurer.getPosition()); if (layout != null)
	 * layouts.push(layout); };
	 * 
	 * measurer.rafaelTextObject.attr("text", layouts.join("\n")); var bb =
	 * measurer.rafaelTextObject.getBBox(); var rect = paper.rect(bb.x+.5, bb.y +
	 * .5, bb.width, bb.height).attr({"stroke-width": 1});
	 * 
	 * lay.push(layouts); console.log(layouts); }
	 */
